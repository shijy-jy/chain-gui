...
- 📖 **阅读模式**（人专用视图，2.17）：工具栏一键把**图结构切成一棵节点文件树**——折叠展开、全库关键字定位（标题/id/标签/正文命中）、「上一篇/下一篇」按文件树前序把整库当一本书读、渲染/原文切换与字号、面包屑与子节点/证据跳转；节点再多也能逐篇读下去（不写工作区任何文件、MCP 无此工具、AI 不可识别不可使用）
