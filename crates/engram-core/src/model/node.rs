...
pub fn apply_update(
    ...
    if let Some(rel) = &fields.rel {
        fm.insert(
            YamlValue::String("rel".into()),
            YamlValue::String(rel.clone()),
        );
    }
    ...
    fm.insert(rev_key, YamlValue::Number(new_rev.into()));

    // 更新 updated 时间
    let now = crate::scanner::frontmatter::now_iso8601();
    fm.insert(YamlValue::String("updated".into()), YamlValue::String(now));
