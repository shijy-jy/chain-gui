...
pub fn validate_fields(filename: &str, fm: &serde_yaml::Mapping, errors: &mut Vec<String>) {
    ...
    if !created.is_empty() && !updated.is_empty() {
        if let (Some(c), Some(u)) = (
            parse_rfc3339_to_epoch(&created),
            parse_rfc3339_to_epoch(&updated),
        ) {
            if u < c {
                errors.push(format!(
                    "[{}] updated: 早于 created（updated={}, created={}）",
                    filename, updated, created
                ));
            }
        }
    }
...
mod tests {
    ...
    fn test_invalid_id_format() {
        ...
        let errors = scan_and_get_errors(&tmp);
        assert!(
            errors.iter().any(|e| e.contains("id: 格式非法")),
            "应报 id 格式错误: {:?}",
            errors
        );
    }
    ...
    fn test_id_filename_mismatch() {
        ...
        let errors = scan_and_get_errors(&tmp);
        assert!(
            errors.iter().any(|e| e.contains("与文件名不匹配")),
            "应报 id 文件名不匹配: {:?}",
            errors
        );
    }
    ...
    fn test_invalid_type_enum() {
        ...
        let errors = scan_and_get_errors(&tmp);
        assert!(
            errors.iter().any(|e| e.contains("type: 非法枚举值")),
            "应报 type 枚举错误: {:?}",
            errors
        );
    }
    ...
    fn test_invalid_status_enum() {
        ...
        let errors = scan_and_get_errors(&tmp);
        assert!(
            errors.iter().any(|e| e.contains("status: 非法枚举值")),
            "应报 status 枚举错误: {:?}",
            errors
        );
    }
    ...
    fn test_empty_title() {
        ...
        let errors = scan_and_get_errors(&tmp);
        assert!(
            errors.iter().any(|e| e.contains("title: 不能为空")),
            "应报 title 空错误: {:?}",
            errors
        );
    }
    ...
    fn test_invalid_created_format() {
        ...
        let errors = scan_and_get_errors(&tmp);
        assert!(
            errors.iter().any(|e| e.contains("created: 不符合 RFC3339")),
            "应报 created 格式错误: {:?}",
            errors
        );
    }
    ...
    fn test_revision_zero() {
        ...
        let errors = scan_and_get_errors(&tmp);
        assert!(
            errors.iter().any(|e| e.contains("revision: 必须为正整数")),
            "应报 revision 非正整数: {:?}",
            errors
        );
    }
    ...
    fn test_tags_not_array() {
        ...
        let errors = scan_and_get_errors(&tmp);
        assert!(
            errors.iter().any(|e| e.contains("tags: 必须为数组")),
            "应报 tags 非数组: {:?}",
            errors
        );
    }
    ...
    fn test_updated_before_created() {
        ...
        let errors = scan_and_get_errors(&tmp);
        assert!(
            errors.iter().any(|e| e.contains("updated: 早于 created")),
            "应报 updated 早于 created: {:?}",
            errors
        );
    }
    ...
    fn test_duplicate_id() {
        ...
        let errors = scan_and_get_errors(&tmp);
        assert!(
            errors.iter().any(|e| e.contains("id: 重复")),
            "应报 id 重复: {:?}",
            errors
        );
    }
    ...
    fn test_dangling_parent() {
        ...
        let errors = scan_and_get_errors(&tmp);
        assert!(
            errors.iter().any(|e| e.contains("parent: 悬空引用")),
            "应报 parent 悬空: {:?}",
            errors
        );
    }
    ...
    fn test_cycle_detection() {
        ...
        let errors = scan_and_get_errors(&tmp);
        assert!(
            errors.iter().any(|e| e.contains("检测到环")),
            "应报环检测: {:?}",
            errors
        );
    }
    ...
    fn test_multiple_roots() {
        ...
        let errors = scan_and_get_errors(&tmp);
        assert!(
            errors.iter().any(|e| e.contains("根节点")),
            "应报多个根节点: {:?}",
            errors
        );
    }
    ...
    fn test_sub_goal_with_parent_no_warning() {
        ...
        assert!(
            !warnings
                .iter()
                .any(|w| w.contains("goal 类型节点不应有 parent")),
            "子 goal 不应报 parent warning: {:?}",
            warnings
        );
    }
