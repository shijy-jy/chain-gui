use crate::model::chain::{ChainHealth, ChainSnapshot, Edge, Manifest, ProjectPersona};
use crate::model::node::Node;
use crate::model::validation::ValidationReport;
use crate::model::ScanMode;
use crate::scanner::frontmatter::{now_iso8601, parse};
...
fn check_guide_staleness(chain_dir: &std::path::Path, warnings: &mut Vec<String>) {
    use crate::guide::{parse_guide_version, AI_GUIDE_VERSION};

    let guide = chain_dir.join("AI_GUIDE.md");
    let result = if !guide.exists() {
        Some(format!(
            "AI_GUIDE.md 缺失（当前软件版本 v{AI_GUIDE_VERSION}）"
        ))
    } else {
...
fn build_active_chain(nodes: &[Node], mode: ScanMode) -> String {
    ...
    /// 树状渲染递归辅助：参数团（节点/子表/前缀/图标闭包）内聚且不可再分，
    /// 8 参数是渲染上下文的直接映射，拆分只会引入无意义的结构体。
...
fn build_project_persona(nodes: &[Node], chain_dir: &std::path::Path) -> Option<ProjectPersona> {
    ...
    // 从根 goal 标题推断领域
    let domain = if root.title.contains("渲染")
        || root.title.contains("shader")
        || root.title.contains("图形")
    {
        "rendering".to_string()
...
mod tests {
    ...
    fn test_chain_health_counts() {
        ...
        let node = |id: &str, node_type: &str, status: &str, title: &str, parent: &str| {
            let parent_line = if parent == "null" {
        ...
        fs::write(
            nodes_dir.join("g-001.md"),
            node("g-001", "goal", "in_progress", "根目标", "null"),
        )
        .unwrap();
        fs::write(
            nodes_dir.join("t-001.md"),
            node("t-001", "task", "success", "任务1", "g-001"),
        )
        .unwrap();
        fs::write(
            nodes_dir.join("t-002.md"),
            node("t-002", "task", "failed", "任务2", "g-001"),
        )
        .unwrap();
        fs::write(
            nodes_dir.join("t-003.md"),
            node("t-003", "task", "blocked", "任务3", "g-001"),
        )
        .unwrap();
        fs::write(
            nodes_dir.join("t-004.md"),
            node("t-004", "task", "pending", "任务4", "g-001"),
        )
        .unwrap();
    ...
    fn test_active_chain_filters_success() {
        ...
        fs::write(
            nodes_dir.join("t-001.md"),
            node("t-001", "task", "success", "已完成任务", "g-001"),
        )
        .unwrap();
        fs::write(
            nodes_dir.join("t-002.md"),
            node("t-002", "task", "pending", "待办任务", "g-001"),
        )
        .unwrap();
        ...
        assert!(
            a.contains("待办任务"),
            "active_chain 应含 pending 节点: {a}"
        );
        assert!(
            !a.contains("已完成任务"),
            "active_chain 不应含 success 节点: {a}"
        );
    ...
    fn test_active_chain_chinese_truncation_no_panic() {
        ...
        for i in 1..=8 {
            ...
            fs::write(
                ...
                node(&id, "task", "pending", &long_title, "g-001"),
            )
            .unwrap();
        }
    ...
    fn test_project_persona_no_false_positive() {
        ...
        let snap = scan_chain_dir(root).unwrap();
        assert!(
            snap.manifest.project_persona.is_none(),
            "无技术栈的通用内容不应生成画像"
        );
    }
    ...
    fn test_dev_mode_allows_multi_root_dangling_cycle() {
        ...
        assert!(
            snap2.validation.valid,
            "环在开发模式应合法: {:?}",
            snap2.validation.errors
        );
