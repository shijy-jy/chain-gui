...
/// v12：骨架浏览——⧉ 全屏展开页
pub const AI_GUIDE_VERSION: u32 = 12;

/// 开发模式指南版本号（与 resources/AI_GUIDE_DEV.md 首行 CHAIN_GUIDE_DEV_VERSION 标记一致）
/// v7：骨架浏览——⧉ 全屏展开页；开发模式指南「过期即刷新」
pub const AI_GUIDE_DEV_VERSION: u32 = 7;
...
pub fn parse_guide_version(content: &str) -> Option<u32> {
    ...
    let num_str = inner.split(':').next_back()?.trim();
    num_str.parse::<u32>().ok()
}

/// 按模式取指南文本：dev → 开发模式知识库指南；其余 → 分析模式链协议指南
/// 分发单点：委托 profile 配置包（终版 §1.4「入口只传 profile 名」）
pub fn guide_for(mode: Option<&str>) -> &'static str {
    crate::profile::profile_for_str(mode).guide
}

/// 按模式取指南版本号
pub fn guide_version_for(mode: Option<&str>) -> u32 {
    crate::profile::profile_for_str(mode).guide_version
}
...
mod tests {
    ...
    fn test_ai_guide_embedded() {
        ...
        assert!(
            AI_GUIDE.contains("title 命名规范"),
            "指南应含 v4 title 命名规范（图谱显示名）"
        );
        assert!(
            AI_GUIDE.contains("LaTeX"),
            "指南应含 v5 数学公式 LaTeX 书写约定"
        );
        // v9/v10：代码骨架内化（M-Code 使用协议，有代码工程必读；骨架挂理论节点）
        assert!(
            AI_GUIDE.contains("代码骨架内化"),
            "指南应含代码骨架内化章节"
        );
        assert!(
            AI_GUIDE.contains("不要为代码模块另建一群骨架节点"),
            "指南应含「不另建骨架节点群」约定"
        );
        assert!(AI_GUIDE.contains("sync-code-map"), "指南应含骨架生成命令");
    }
    ...
    fn test_ai_guide_dev_embedded() {
        ...
        assert!(
            AI_GUIDE_DEV.contains(".chain/.mode = dev"),
            "应含模式标签说明"
        );
        assert!(
            AI_GUIDE_DEV.contains("Zettelkasten"),
            "应含优秀实践参考（卡片盒）"
        );
        assert!(AI_GUIDE_DEV.contains("不可混用"), "应含两模式不可混用规则");
        // v2：递进关系建模（rel 字段）
        assert!(
            AI_GUIDE_DEV.contains("递进关系建模"),
            "应含 v2 递进关系建模章节"
        );
        assert!(AI_GUIDE_DEV.contains("rel"), "应含 rel 关系类型字段说明");
        assert!(AI_GUIDE_DEV.contains("solves"), "应含 solves 关系类型");
        // v4：代码骨架内化（M-Code 使用协议）
        assert!(
            AI_GUIDE_DEV.contains("骨架挂理论节点"),
            "应含 v5 骨架挂理论节点章节"
        );
        assert!(
            AI_GUIDE_DEV.contains("不要为代码模块另建一群骨架节点"),
            "应含「不另建骨架节点群」约定"
        );
        assert!(
            AI_GUIDE_DEV.contains("code_map"),
            "应含 code_map 挂载说明"
        );
        assert!(
            AI_GUIDE_DEV.contains("sync-code-map"),
            "应含骨架生成命令"
        );
        assert!(
            AI_GUIDE_DEV.contains("stale 兜底"),
            "应含 stale 刷新协议"
        );
    }
    ...
    fn test_parse_guide_version_old_style() {
        ...
        // 非法数字 → None
        assert_eq!(
            parse_guide_version("<!-- CHAIN_GUIDE_VERSION: abc -->"),
            None
        );
    }
    ...
    fn test_parse_guide_version_v1() {
        assert_eq!(
            parse_guide_version("<!-- CHAIN_GUIDE_VERSION: 1 -->"),
            Some(1)
        );
        assert_eq!(
            parse_guide_version("<!-- CHAIN_GUIDE_VERSION: 3 -->"),
            Some(3)
        );
        assert_eq!(
            parse_guide_version("<!-- CHAIN_GUIDE_DEV_VERSION: 1 -->"),
            Some(1)
        );
    }
}
