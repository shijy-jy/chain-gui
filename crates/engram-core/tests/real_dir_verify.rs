...
use engram_core::guide::{parse_guide_version, AI_GUIDE, AI_GUIDE_VERSION};
use engram_core::model::node::NodeStatus;
use engram_core::model::{ScanMode, UpdateFields};
use engram_core::ops::chain::{
    append_log, fold_chain, get_process_log, init_chain, list_snapshots, read_snapshot,
    refresh_ai_guide_if_stale, snapshot_chain,
};
use engram_core::ops::node_edit::update_node_fields;
use std::fs;
use std::path::Path;
...
fn test_full_flow_on_real_dir() {
    ...
    let snap = init_chain(root, ScanMode::Analysis).unwrap();
    ...
    let snap2 = init_chain(root, ScanMode::Analysis).unwrap();
    ...
    let snap3 = update_node_fields(root, "g-001", &fields, ScanMode::Analysis).unwrap();
    ...
    // 4. 过程日志：追加两条 + 读回
    append_log(root, "验证：环境坑一条").unwrap();
    append_log(root, "验证：失败尝试一条\n多行合并").unwrap();
    let log = get_process_log(root).unwrap();
    assert!(log.contains("环境坑一条"));
    assert!(log.contains("失败尝试一条；多行合并"));

    // 5. v1.3 快照：创建/列表/读取
    let snap_id = snapshot_chain(root, "验证快照").unwrap();
    let snaps = list_snapshots(root).unwrap();
    assert_eq!(snaps.len(), 1);
    assert_eq!(snaps[0].tag, "验证快照");
    let restored = read_snapshot(root, &snap_id).unwrap();
    assert_eq!(restored.nodes.len(), 1);

    // 6. v1.3 manifest 新字段：active_chain + chain_health + project_persona
    let snap4 = engram_core::scanner::walker::scan_chain_dir(root).unwrap();
    assert!(
        snap4.manifest.active_chain.contains("示例目标"),
        "active_chain 应含根: {}",
        snap4.manifest.active_chain
    );
    ...
    let after_fold = fold_chain(root, "d-001", ScanMode::Analysis).unwrap();
    ...
    assert!(
        root.join(".chain/archive/fold_d-001/_self.md").exists(),
        "目标节点应有 _self.md 备份"
    );

    // 8. 指南陈旧检测：手写 v1 标记 → refresh 应刷新到当前版本
    fs::write(
        root.join(".chain/AI_GUIDE.md"),
        "<!-- CHAIN_GUIDE_VERSION: 1 -->\n旧版",
    )
    .unwrap();
    ...
    assert!(fs::read_to_string(root.join(".chain/AI_GUIDE.md"))
        .unwrap()
        .contains("我的批注"));
