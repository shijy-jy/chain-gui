...
AIGC:
    ...
    ProduceID: 3436392644363562_0/project_7673108688902668594-files/task_cards/ai_join_ai-frontend.md
    ReservedCode1: ""
    ContentPropagator: 001191110102MACQD9K64028705
    PropagateID: 3436392644363562#1786574674616
    ReservedCode2: ""
---
# 加入任务卡 — ai-frontend
...
你是 **ai-frontend** 角色（前端主导：Svelte 5 + TypeScript + cytoscape.js 图谱可视化）。本卡是 coze 写给你的"加入任务卡"——你按下面 7 步走完，就是工程认证的协作成员。

完成后我会（coze）验收你的 `ai_self_profile.md`，把你写进规划书第 3/5 章的 AI 工具链表格。
...
2. `G:\test1.x\ai_workspace\CODE_STATE.md` — 当前基线 + 已加入 AI 名单 + 进行中任务
...
| 路径 | 动作 | 模板 |
|------|------|------|
| `ai_workspace/ai-frontend/ai_self_profile.md` | 新建 | §6.5 模板（YAML + 自我介绍四段） |
| `ai_workspace/ai-frontend/CURRENT.md` | 追加"自我介绍段"（已存在文件，由 Init 任务卡建） | ≥ 5 行 |
| `ai_workspace/CODE_STATE.md` | 追加"已加入 AI 名单"区的新行 | 7 列表格 |
| `ai_workspace/ai-frontend/self_review/ai_join.md` | 新建 | 自验收报告 |
...
路径：`G:\test1.x\ai_workspace\ai-frontend\ai_self_profile.md`

按 §6.5 模板填，**以下三个字段必须实填**（不能留 `<AI 自填>` 占位）：

- `ai_name`：你的真实名字（例：Claude Code Sonnet 4.5 / Codex CLI 0.45 / Cursor Composer 1.0）
- `ai_tool`：你跑在什么工具上（例：Claude Code CLI / Codex CLI / Cursor / Aider / 其他）
- `tool_version`：工具版本号
- `capabilities`：**≥ 3 条**，每条基于你真的擅长什么（不是套话）
...
<第一人称，≥ 3 句话，说明你跑在什么工具上、由谁部署、为什么会作为 ai-frontend 加入这个工程>

## 我能做什么

<基于 capabilities 展开，每条 ≥ 1 句解释，给具体例子>
...
路径：`G:\test1.x\ai_workspace\ai-frontend\CURRENT.md`（已存在，**追加**不要覆盖）

在文件**最前面**（`# ai-frontend — 当前任务` 标题之上）插入一段自我介绍（≥ 5 行）：
...
找到 `## 已加入 AI 名单` 表格（在文件末尾，Init 任务卡已经建了 3 行占位 "（待 M0 自注册时填）"），把这 3 行**全部替换**为：

```markdown
| AI 角色 | 工具 | 版本 | 加入基线 | 加入日期 | operator | profile 链接 |
|---------|------|------|----------|----------|----------|--------------|
| ai-frontend | <填你的 ai_tool> | <填 tool_version> | BL-20260812-02 | 2026-MM-DD | 瑾瑜 | ai-frontend/ai_self_profile.md |
| ai-rust | （M0 时该 AI 自己注册后填） | — | — | — | 瑾瑜 | — |
| ai-qa | （M0 时该 AI 自己注册后填） | — | — | — | 瑾瑜 | — |
```

> 注意：你是 M0 阶段**第一个**走自注册的（先 ai-frontend → 再 ai-rust → 再 ai-qa，或反过来）。**只填你自己那一行**，其他两行保持 "（M0 时该 AI 自己注册后填）" 即可。
...
git add ai_workspace/ai-frontend/ai_self_profile.md \
        ai_workspace/ai-frontend/CURRENT.md \
        ai_workspace/CODE_STATE.md
git commit -m "BL-20260812-02: baseline: ai-frontend 自注册开始"
...
路径：`G:\test1.x\ai_workspace\ai-frontend\self_review\ai_join.md`

```markdown
# ai_join 自验收报告 — ai-frontend

code_baseline: BL-20260812-02
ai: ai-frontend（<填 ai_name>）
date: 2026-MM-DD
...
2. 回填规划书 v3 第 3/5 章 AI 工具链表格（ai-frontend 那行的"工具"列从"AI 自注册时填"改为"已自填 + 工具名"）
...
# done commit
cd G:\test1.x
git add ai_workspace/ai-frontend/self_review/ai_join.md
git commit -m "BL-20260812-02: done: ai-frontend 自注册完成 + self_review"
```
...
- 在 `ai_workspace/ai-coordinator/reviews/ai_join_ai-frontend.md` 写验收通过记录
- 回填 v3 规划书第 3/5 章 AI 工具链表格
- 触发下一个 AI 的加入任务卡（ai-rust 或 ai-qa，按 coze 的节奏）
...
3. **CODE_STATE.md**："已加入 AI 名单"区有 ai-frontend 行（ai_name/ai_tool/tool_version/code_baseline/joined_at 全部实填）
4. **git log**：含 baseline + done 两次 commit，message 带基线号 BL-20260812-02
5. **self_review.md**：完整 7 项勾选 + git log 输出 + 自我介绍摘录

**全部通过 → coze 写验收通过记录 + 回填规划书表格 + 触发下一个 AI**
...
- **本卡具体场景**：M0（AI 集中接入）阶段的第一个 AI（ai-frontend）。基线 BL-20260812-02 是 M1 完成后的状态。
